...
Description: demo file that shows a quick portion of the pitch detection and music transcription portion of the
             project. Simply run the program in the terminal as "python demo.py" and chose a song (start with one of
             the samples from the zip download). Once the program has finished, look in the main directory for the
             "demo_output.mid" file. This is the generated MIDI file containing the transcription of the song you chose.
             We recommend opening this MIDI file in MuseScore as you can see the sheet music transcription, but any
             online MIDI file reader will also work, you just probably won't see the sheet music.

Dependencies Needed: (Refer to README for installation procedure)
- numpy
- scipy
- librosa
- pretty_midi

(Note: this is a duplicate of the Sample_Polyphonic_Transcription.py file)
"""
...
def Transcribe(FILES: list[str], instrumentTypes: list[int] = None, saveName: str = '', verbose=False,
               Output_Folder='') -> str:
    """
...
def GenerateInstrument(filepath: str, instrument_type=1, instrument_name='piano', is_drum=False,
                       verbose=False) -> pretty_midi.Instrument:
    """
